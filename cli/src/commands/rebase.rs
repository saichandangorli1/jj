...
/// so if you instead run `jj rebase -s M -s N -o O` (or
/// `jj rebase -s 'all:M|N' -o O`) in the example above, then N' would instead
...
/// targets. Existing descendants of the targets will not be affected. See the
